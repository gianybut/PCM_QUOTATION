    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4 mb-4 non-printable">
          <button onClick={addRow} className="bg-amber-300 text-black px-4 py-2 rounded hover:bg-amber-400">
            Add Row
          </button>

          <div className="flex items-center gap-2 non-printable">
            <input type="text" value={newUnit} onChange={(e) => setNewUnit(e.target.value)} placeholder="Add new unit" className="border rounded px-2 py-1" />
            <button onClick={handleAddUnit} className="bg-amber-300 text-black px-2 py-1 rounded hover:bg-amber-400">
              <Plus className="w-4 h-4" />
            </button>
          </div>

          <div className="flex items-center gap-2 non-printable">
            <input type="text" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Add new description" className="border rounded px-2 py-1" />
            <button onClick={handleAddDescription} className="bg-amber-300 text-black px-2 py-1 rounded hover:bg-amber-400">
              <Plus className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      <div className="overflow-x-auto table-div">
        <table className="border-collapse border-hidden w-full bg-white shadow-md overflow-hidden rounded-lg">
          <thead>
          ...
          <tbody className='bg-slate-100 border-hidden table-auto overflow-hidden rounded-lg print:page-break-inside-avoid'> {/* Key change here */}
            {tableData.map(row => (
              <tr key={row.id} className="print:page-break-inside-avoid"> {/* Key change here */}
                ...
                <td className="border p-2 border-hidden">
                  <div className="flex items-center">
                    ...
                    <input type="number" min="0" step="0.01" value={row.pricePerUnit} onChange={(e) => handleChange(row.id, 'pricePerUnit', parseFloat(e.target.value) || 0)} className="w-full p-2 border-b-1 text-center" />
                  </div>
                </td>
                <td className="border p-2 border-hidden">
                  {formatPeso(row.total)}
                </td>
                <td className="border p-2 border-hidden non-printable">
                  <button onClick={() => deleteRow(row.id)} className="text-red-500 hover:text-red-900 p-1 non-printable" disabled={tableData.length === 1} title={tableData.length === 1 ? "Cannot delete last row" : "Delete row"}>
